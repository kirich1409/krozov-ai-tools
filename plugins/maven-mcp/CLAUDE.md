...
**Userinfo redaction.** Repo URLs are captured verbatim from build files (see *Documented limitations*), so a discouraged hardcoded `url = "https://user:pass@host/repo"` would otherwise echo the literal credential into MCP tool-facing JSON. `_strip_userinfo(url)` redacts `user:pass@` to `***@` at the output boundary only (it never touches the raw URL used for the actual HTTP fetch) and is applied to every field that can carry a repo URL or repo name: `resolvedFrom.url` (via `_to_resolved_from`), `check_version_exists`'s `repository` field, `verify_coordinates`'s `repository` field, and `fetch_metadata`'s non-200 `last_err` message (which otherwise flows the same unredacted repo name/URL into the `error`/`healthError` fields of every handler whose fetch ultimately fails) — `maven("url")` / `maven { url = ... }` declarations set the repo's `name` equal to its `url` (see `discover_repositories`), so any field carrying a repo name carries the same exposure as `resolvedFrom.url`. Best-effort: `_strip_userinfo` redacts the URL-authority form (`scheme://user:pass@host`); a scheme-less or malformed-host URL that fails `urllib.parse.urlsplit` is returned unredacted, but such a URL also cannot successfully complete an HTTP fetch in this codebase (no valid scheme/host to connect to), so it never reaches the success/`resolvedFrom` path — only a theoretical, currently-unreachable gap in the transport-exception message branch.
