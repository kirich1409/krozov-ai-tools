...
- **Deferred #299 pieces** (this layer addresses the core; the rest are follow-ups): content / group filtering — #320. (Parent-POM / Maven-profile inheritance was #319, resolved — see *Maven parent-POM and profile repository inheritance* above; the residual gaps noted there — `settings.xml` profile activation, non-`activeByDefault` activation conditions, network-fetching an unresolvable-locally parent — remain open.)
