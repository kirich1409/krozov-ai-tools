# Step 4 — Aggregation and Receipt Format
...
Step 2.5 writes a stub for skipped `code-reviewer`; Step 3.3 writes an artifact even on
build-smoke failure. If a planned per-check artifact is nonetheless missing at aggregation
time, treat the check as `verdict: FAIL` with `blocked_on: per-check artifact missing` — do
not silently drop it. `blocked_on` is the canonical field for surfacing unresolved
conditions per the per-check schema; no separate `error:` field exists.
...
The final `VERIFIED | FAILED | PARTIAL` decision table lives in `SKILL.md` §Step 4.
The rules above feed into it — a PoLL blocker maps to `FAILED`, PoLL important maps to
`PARTIAL`, and so on.
