...
- права: принудительно `0644`. Это **не всегда** то, что пишет `mcpb pack`: он копирует режим из
  stage-дерева, поэтому исполняемый файл приехал бы с `0755`, и бит исполнения здесь снимается.
  Снимается намеренно: всё содержимое бандла — данные, которые читает интерпретатор (точка входа
  запускается как `python3 server/server.py`), исполняемых записей в нём нет. Бандлу, которому
  исполняемая запись понадобится, придётся править нормализатор, а не обходить его;
- сжатие: `STORE` для всех записей, `create_system` = 3 (Unix). DEFLATE не используется осознанно:
  его вывод — свойство конкретной сборки zlib, то есть дайджест воспроизвёл бы только тот, у кого
  zlib совпал с релизным раннером. Цена размена названа прямо: бандл вырос с ~77 КБ до ~194 КБ, зато
  байты даёт любой Python на любой платформе;
- запись, у которой выставлены биты типа файла, отличные от regular (например симлинк),
  **отклоняется**, а не переписывается молча в обычный файл. Измеренная оговорка, чтобы это не
  принимали за границу защиты от симлинков: `mcpb pack` симлинки **разыменовывает** (симлинк на
  `/etc/passwd` уезжает обычной записью с содержимым этого файла) и всегда пишет голую маску прав с
  нулевыми битами типа, так что на архиве этой пайплайны проверка сработать не может — это запас
  прочности против будущего упаковщика, сохраняющего симлинки. Реальный containment дают
  `find -type l` в `pack-mcpb.sh` (шаги 5 и 10) **до** упаковки, и ослаблять их, ссылаясь на
  нормализатор, нельзя;
- также отклоняются: directory-запись (`mcpb pack` их не пишет вовсе, а корректно собрать такую
  запись — непроверяемый код), небезопасное или не переживающее round-trip имя (NUL, обратный слэш,
  абсолютный путь, `..`) и **байты, дописанные за EOCD**. Последнее — про подпись: подпись MCPB это
  PKCS#7-блок, приклеенный за End Of Central Directory, а не zip-запись, поэтому пересборка архива
  сняла бы её без единого сообщения. Порядок шагов, который отсюда следует: stage-гейты →
  `mcpb validate` → `mcpb pack` → нормализация → [будущая подпись] → чексумма → подстановка в
  `dist/`.
...
Что гарантируется и что нет. Гарантируется: сборка из одного и того же дерева исходников даёт один
и тот же файл, поэтому SHA-256 релизного бандла можно воспроизвести локально и сверить. Записи
хранятся без сжатия, поэтому от версии Python, платформы и реализации zlib байты не зависят.
Единственное внешнее условие — та же версия `mcpb` CLI: от неё зависит состав записей архива
(содержимое `manifest.json` формирует шаг 9 `pack-mcpb.sh` из шаблона репозитория, CLI его только
валидирует). Рецепт пересборки — в `README.md`, раздел проверки бандла.

Известное ограничение: воспроизводимость доказывается **внутри одного джоба** — две сборки подряд на
одной машине (`test-mcpb-reproducible.sh` в CI-джобе `youtube-transcript-mcpb`). Что дайджест
релизного бандла совпадает с дайджестом сборки того же коммита в CI, **нигде не сверяется**: `pack`
в `release.yml` собирает заново и ни с чем не сравнивает. Значит доказан детерминизм сборки, а не
соответствие релизных байт тем, что видел CI; дрейф релизного окружения такой сверкой не ловится,
потому что сверки нет. Provenance-аттестат этот пробел не закрывает: он подтверждает происхождение
байт, а не их соответствие исходникам.

Чексумма при этом по-прежнему **не** контроль подлинности: `.mcpb` и `.sha256`
