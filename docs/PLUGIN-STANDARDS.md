...
**Бандл не воспроизводим побайтно — измерено.** На `main@9ea164a`, `@anthropic-ai/mcpb@2.1.2`,
версия плагина `0.1.0`:

- две подряд сборки `bash scripts/pack-mcpb.sh` на неизменном входе дают **разные** SHA-256:
  `mcpb pack` вшивает в архив mtimes файлов;
- `SOURCE_DATE_EPOCH=1700000000 bash scripts/pack-mcpb.sh` — де-факто стандарт воспроизводимых
  сборок — **игнорируется**: две сборки под ним тоже разошлись. Не документировать его как
  средство;
- пост-обработка архива (перезапись с сортировкой entries, `date_time` = ZIP-эпоха 1980-01-01,
  фиксированный внешний атрибут `0644`, `ZIP_DEFLATED`) даёт **побайтно одинаковый** результат для
  обеих сборок, и нормализованный бандл остаётся валидным —
  `bash scripts/smoke-mcpb.sh --require-checksum <bundle>` печатает
  `smoke-mcpb: all 8 assertions passed`, включая живой MCP-хендшейк.

Полный протокол измерения — `swarm-report/mcpb-stage2-t8-reproducibility.md`.

Следствие для чексуммы: опубликованный `.sha256` подтверждает только, что скачанные байты равны
залитым (защита от повреждения при передаче). Он **не** подтверждает, что бандл собран из данного
дерева исходников, и не является контролем подлинности — подлинность даёт build provenance. Любая
будущая схема, сверяющая CI-сборку с релизной, обязана это учитывать: сравнивать содержимое, а не
хэш архива целиком. Нормализация архива в `pack-mcpb.sh` — обоснованный следующий шаг с уже
проведённым экспериментом, но это изменение отгружаемого артефакта, и здесь оно намеренно не
сделано.
